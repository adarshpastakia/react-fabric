...
export class NodeCirclePie extends NodeProgram<
  (typeof UNIFORMS)[number],
  NodeAttributes & { opacity: number },
  EdgeAttributes
> {
  ...
  getDefinition() {
    return {
      ...
      CONSTANT_DATA: [
        [NodeCirclePie.ANGLE_1],
        [NodeCirclePie.ANGLE_2],
        [NodeCirclePie.ANGLE_3],
      ],
    };
  }
  ...
  processVisibleItem(
    nodeIndex: number,
    startIndex: number,
    data: NodeDisplayData & NodeAttributes & { bg: string; opacity: number },
  ) {
