  ...
  return (
    ...
    for (let i = 0, length = label.length; i < length; i++) {
      ...
      if (!charactersLengthCache[character]) {
        charactersLengthCache[character] =
          context.measureText(character).width * (1 + curvature * 0.35);
      }
    }
    ...
    for (let i = 0, length = label.length; i < length; i++) {
      ...
      const tangentX =
        2 * (1 - t) * (anchorX - sourceX) + 2 * t * (targetX - anchorX);
      const tangentY =
        2 * (1 - t) * (anchorY - sourceY) + 2 * t * (targetY - anchorY);
