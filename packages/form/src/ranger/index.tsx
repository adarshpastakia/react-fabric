...
import { getBoundingClientRect, linearInterpolator, sortNumList } from "./utils";
...
export function useRanger(opts: RangerConfig) {
  ...
  const positionProp = useMemo(() => (isVertical ? "bottom" : isRtl ? "right" : "left"), [isVertical, isRtl]);
  const marginProp = useMemo(() => (isVertical ? "marginBottom" : isRtl ? "marginRight" : "marginLeft"), [isVertical, isRtl]);
  ...
  const getValueForClientX = useCallback(
    (clientX: number) => {
      ...
      return linearInterpolator.getValueForClientX(clientX, trackDims, min, max, isRtl);
    },
    [max, min, isRtl],
  );
  const getValueForClientY = useCallback(
    (clientY: number) => {
      const trackDims = getBoundingClientRect(trackElRef.current);
      return linearInterpolator.getValueForClientY(clientY, trackDims, min, max);
    },
    [max, min],
  );
  ...
  const handleDrag = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const newValue = isVertical ? getValueForClientY(clientY) : getValueForClientX(clientX);
    ...
    [getLatest, getValueForClientX, getValueForClientY, roundToStep, values, isVertical],
  );
  const handleSegmentDrag = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const start = isVertical ? getValueForClientX(bottom + (clientY - segY)) : getValueForClientX(left + (clientX - segX));
    ...
    [getLatest, getValueForClientX, getValueForClientY, roundToStep, values, isVertical, max],
  );
  ...
  const handlePress = useCallback(
    (e: ME | TE, i: number) => {
      ...
      const handleRelease = () => {
        ...
        document.removeEventListener("mousemove", i === -1 ? handleSegmentDrag : handleDrag);
        document.removeEventListener("touchmove", i === -1 ? handleSegmentDrag : handleDrag);
      ...
      onDragStart?.();
      document.addEventListener("mousemove", i === -1 ? handleSegmentDrag : handleDrag);
      document.addEventListener("touchmove", i === -1 ? handleSegmentDrag : handleDrag);
  ...
  const trackClicked = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      if (values.length === 2) {
        if (values[0] > newRoundedValue) newValues = [newRoundedValue, values[1]];
        else if (values[1] < newRoundedValue) newValues = [values[0], newRoundedValue];
        else {
          const diff = (values[1] - values[0]) / 2;
          if (values[0] + diff >= newRoundedValue) newValues = [newRoundedValue, values[1]];
          if (values[1] - diff <= newRoundedValue) newValues = [values[0], newRoundedValue];
        }
      }
      onChange(newValues);
    },
  ...
  }, [controlledTicks, getPercentageForValue, max, min, steps, tickSize, positionProp, marginProp]);
  ...
  }, [getPercentageForValue, max, min, tempValues, values, positionProp, marginProp]);
